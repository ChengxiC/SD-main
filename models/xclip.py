...
from models.prompt import VideoSpecificPrompt
from models.cct import CrossFrameCommunicationTransformer
...
class XCLIP(CLIP):
    ...
    def forward(self, image, text):
        ...
        if self.training:
            ...
            logits = torch.einsum("bd,bkd->bk", v_features, logit_scale * t_features)  # 视频特征与文本特征之间的相似度 logits
            logits_u = torch.einsum("bd,bkd->bk", v_features_u, logit_scale * t_features)
...
def load(model_path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
    ...
    if state_dict is not None:  # 如果字典为空的话，加载状态字典
        model = build_model(state_dict['model'], T=T, droppath=droppath,
        ...
        model = build_model(model.state_dict(), T=T, droppath=droppath,
                            use_checkpoint=use_checkpoint, logger=logger,
                            prompts_alpha=prompts_alpha,
                            prompts_layers=prompts_layers,
                            use_cache=use_cache,
