...
def build_optimizer(config, model):
    ...
    clip_parameters = set_weight_decay(
    ...
    msg_parameters = set_weight_decay(
        model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR,
        have=("message_",), not_have=()
    )
    mit_parameters = set_weight_decay(
        model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR,
        have=("mit",), not_have=()
    )
    prompts_parameters = set_weight_decay(
        model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR,
        have=("prompts",), not_have=()
    )
    anomaly_parameters = set_weight_decay(
        model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR*0.1,
        have=("anomaly_head",), not_have=()
    )
    cluster_parameters = set_weight_decay(
        model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR,
        have=("cluster_head",), not_have=()
    )
    optimizer = optim.AdamW(
        clip_parameters + mit_parameters + prompts_parameters + msg_parameters + anomaly_parameters,
        betas=(0.9, 0.98), eps=1e-8,
    )

    optimizer_umil = optim.AdamW(cluster_parameters + anomaly_parameters, betas=(0.9, 0.98), eps=1e-8,)
