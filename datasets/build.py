...
class RawFramesTestRecord(object):
    ...
    # 通过以下三个属性 path, num_frames, label 提供对数据的访问.
    @property
    def path(self):
        return self._data[0]
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                if self.multi_class:   # False
                    ...
                    if len(line_split) == 4:  # 视频名， 开始帧序号，中止帧序号，视频标签
                        filename, start, end, label = line_split
                    elif len(line_split) == 5:  # 视频名，中止帧序号，视频标签
                ...
                vid += 1
        return video_infos  # 包含字典的列表，每个字典包含一个视频信息.
...
class RawFramesTestDataset(BaseDataset):
    ...
    def load_annotations(self):
        ...
        for x in open(self.ann_file):
            ...
            num_segs = math.ceil(video_info.num_frames / self.seg_interval)  # 向上取整，确保覆盖所有检测的帧
...
def build_dataloader(logger, config):
    ...
    train_loader = DataLoader(
        ...
        num_workers=6,  # 之前16g内存条, windows不能设置为6或8或者更大.
        pin_memory=True,
        drop_last=True,
    ...
    test_pipeline = [
        # 这里之所以取num_clips=1是因为后面的RawFramesTestDataset对视频进行标注，一个视频30帧，一个clip,
        # 如：[{'frame_dir': 'E:\\datasets\\TAD_reorganized\\frames/abnormal/01_Accident_006.mp4',
        #   'label': 1,
        #   'start': 0,
        #   'total_frames': 30,
        #   'tar': False,
        #   'vid': 0},
        #  {'frame_dir': 'E:\\datasets\\TAD_reorganized\\frames/abnormal/01_Accident_006.mp4',  # 作为一个小视频
        #   'label': 1,
        #   'start': 30,
        #   'total_frames': 30,
        #   'tar': False,
        #   'vid': 0},
        #  {'frame_dir': 'E:\\datasets\\TAD_reorganized\\frames/abnormal/01_Accident_006.mp4',
        dict(type='SampleFrames', clip_len=config.DATA.NUM_FRAMES, frame_interval=config.DATA.FRAME_INTERVAL, num_clips=1, test_mode=True),
        dict(type='RawFrameDecode'),
        dict(type='Resize', scale=(-1, scale_resize)),
        dict(type='CenterCrop', crop_size=config.DATA.INPUT_SIZE),
        dict(type='Normalize', **img_norm_cfg),
    ...
    test_loader = DataLoader(
        ...
        batch_size=64,
        num_workers=6,   # 我的电脑不能设置为6或8或者更大?
        pin_memory=True,
        drop_last=False,
